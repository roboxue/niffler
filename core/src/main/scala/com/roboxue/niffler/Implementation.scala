...
/**
  ...
  * @since 12/15/17.
  */
sealed case class Implementation[T](key: Key[T], sketch: ImplementationSketch[T])

abstract class ImplementationDetails[T](val dependency: Set[Key[_]]) extends ImplementationSketch[T] {
  private[niffler] def forceEvaluate(cache: ExecutionCache): T
}

abstract class ImplementationIncrement[T](val dependency: Set[Key[_]]) extends ImplementationSketch[T] {
