...
object IterationTrainer extends Niffler {
  ...
  val iterationTrainingData: Token[Seq[DecaIndexedInput]] = Token("a collection of preprocessed question and contexts." +
  ...
  override val logic = new Logic(Seq(
    performTraining.dependsOn(modelStartState, iterationLearningRate, iterationTrainingData, vocabulary).implBy({
      (model, lr, data, vocab) =>
        ???
    }),
    saveSnapshot.dependsOn(modelSavingPath, iterationId).implBy({
      (basePath, id) =>
